...
type Props = {|
  ...
  yScaleType: string,
  /** Override the default scale type for the R axis. */
  rScaleType: string,
...
class BubbleChart extends PureComponent<Props, State> {
  ...
  updateDimensions = () => {
    ...
    if (fluid) {
      this.setState({
        width: (this.bubbleChart && this.bubbleChart.offsetWidth) || width,
      });
    }
  };
  ...
  render() {
    ...
    const { w, h, m, x, y, r } = chart.calculateChartValues(
